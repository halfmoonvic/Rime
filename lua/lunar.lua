...
function GregorianToLunar(nSYear, nSMonth, nSDate)
  ...
  local nongLi = shuXing ..
      '(' .. nLTianGan[(((nLYear - 4) % 60) % 10) + 1] .. nLDiZhi[(((nLYear - 4) % 60) % 12) + 1] .. ')年'
  local nLDate
  --生成农历月、日 ==> nLDate--*/
  if nLMonth < 1 then
...
local function translator(input, seg)
  date_y = tonumber(os.date("%Y"))
  date_m = tonumber(os.date("%m"))
  date_d = tonumber(os.date("%d"))

  nLYear, nLMonth, nLDay, nLDate = GregorianToLunar(date_y, date_m, date_d)
  date = nLYear .. "年" .. nLMonth .. "月" .. nLDay .. "日"

  if (input == "lunar") then
    yield(Candidate("lunar", seg.start, seg._end, nLDate, "农历"))
