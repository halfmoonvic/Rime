...
--节气表
local jqB = { "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑",
  "大暑", "立秋", "处暑", "白露", "秋分", "寒露", "霜降", "立冬", "小雪", "大雪", "冬至", "小寒",
  "大寒" }
--天干
...
function lunarJzl(y)
  ...
  if tonumber(y.sub(y, 1, 4)) > 1970 then
      ...
      os.time(
        {
          ...
          month = tonumber(y.sub(y, 5, 6)),
          day = tonumber(y.sub(y, 7, 8)),
          hour = tonumber(y.sub(y, 9, 10)),
          min = 4,
          sec = 5
        }
      )
    ...
    GzData = get60JiaZiStr(yidx) ..
        "年" .. get60JiaZiStr(midx) .. "月" .. get60JiaZiStr(didx) .. "日" .. get60JiaZiStr(hidx) .. "时"
    return GzData
  else
    return "不支持1971年前的日期"
  end
